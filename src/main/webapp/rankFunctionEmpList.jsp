...
<%
	...
	*/
	String sql = "SELECT 글번호, 사원아이디, 사원이름, 급여, 급여순위 FROM(SELECT rownum 글번호, employee_id 사원아이디, last_name 사원이름, salary 급여, rank() over(order by salary desc) 급여순위 FROM employees) WHERE 글번호 BETWEEN ? AND ?";
...
<body>
	...
	<%
		...
		if(beginPage != 1) {
	%>
			<a href="<%=request.getContextPath()%>/rankFunctionEmpList.jsp?currentPage=<%=beginPage - 1%>">이전</a>
	<%
		}
	%>
	...
	<%
		...
		if(endPage != lastPage) {
	%>
			<a href="<%=request.getContextPath()%>/rankFunctionEmpList.jsp?currentPage=<%=endPage + 1%>">다음</a>
	<%
		}
	%>
</body>
